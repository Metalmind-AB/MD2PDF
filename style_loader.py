...
class StyleLoader:
    ...
    def discover_styles(self) -> Dict[str, Dict]:
        ...
        for css_file in self.styles_dir.glob("*.css"):
            ...
            styles[style_name] = {
                ...
                'description': style_info.get('description', f'{style_name.title()} style'),
                'file': str(css_file),
    ...
    def discover_themes(self) -> Dict[str, Dict]:
        ...
        for css_file in self.themes_dir.glob("*.css"):
            ...
            themes[theme_name] = {
                ...
                'description': theme_info.get('description', f'{theme_name.title()} theme'),
                'file': str(css_file),
    ...
    def get_style_css(self, style_name: str) -> str:
        ...
        if style_name not in styles:
            raise ValueError(f"Style '{style_name}' not found. Available: {list(styles.keys())}")
    ...
    def list_styles(self) -> List[Tuple[str, str, str]]:
        ...
        styles = self.discover_styles()
        return [(name, info['name'], info['description']) for name, info in styles.items()]
    
    def list_themes(self) -> List[Tuple[str, str, str]]:
        """List all available themes with their descriptions."""
        themes = self.discover_themes()
        return [(name, info['name'], info['description']) for name, info in themes.items()]
