...
def test_installation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Test --version
        result = run_command(
            [str(python_path), "-m", "md2pdf", "--version"],
            capture=True
        )
        if result.returncode != 0:
        ...
        # Test --list-styles
        result = run_command(
            [str(python_path), "-m", "md2pdf", "--list-styles"],
            capture=True
        )
        if result.returncode != 0:
        ...
        # Test --list-themes
        result = run_command(
            [str(python_path), "-m", "md2pdf", "--list-themes"],
            capture=True
        )
        if result.returncode != 0:
...
""")
        ...
        result = run_command(
            [str(python_path), "-m", "md2pdf", str(test_md), 
             "-o", str(temp_path / "test.pdf"),
             "--style", "technical",
             "--theme", "github"],
            capture=True
        )
        ...
        if result.returncode == 0:
            ...
            core_deps = ['markdown', 'weasyprint', 'pygments', 'beautifulsoup4', 'click', 'rich']
            for pkg in packages:
